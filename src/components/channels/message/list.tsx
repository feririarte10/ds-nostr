              ...
              <div key={`${eventMessage.id}-${index}`} className='message'>
                ...
                <div className='message-info'>
                  <div className='message-heading'>
                    <span className='message-author'>{formatAddress(nip19.npubEncode(eventMessage.pubkey), 12)}</span>
                    <span className='message-date'>
                      {new Date(eventMessage.created_at * 1000).toLocaleDateString()} -{' '}
                      {new Date(eventMessage.created_at * 1000).toLocaleTimeString()}
                    </span>
                  </div>
                  <div className='message-content'>{parsedContent}</div>
                </div>
              </div>
            );
          })}
